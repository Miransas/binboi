...
Binboi now has two explicit operating modes:

- local preview mode for intentional developer-only testing
- full account-backed mode for the real product path

Preview mode is no longer meant to be an invisible production fallback. If you want preview behavior without Postgres auth, enable it deliberately with `BINBOI_ALLOW_PREVIEW_MODE=true`.
...
- It keeps local preview mode available for intentional testing while making the account-backed deployment path the default serious setup.
...
The repository ships with two checked-in project metadata files:

- [`miransas.json`](./miransas.json): organization and project-level manifest
- [`binboi.json`](./binboi.json): product/runtime manifest

Anyone downloading the project gets these files immediately at the repo root. When versions change, refresh them with:

```bash
make sync-manifests
```

These manifests now track:

- project and repository identity
- CLI/web/runtime versions
- release channel and API version
- release date and minimum CLI version
- deploy modes and build targets

## Deployment Templates

Production and staging starting points now live here:

- [`configs/binboi.production.env.example`](./configs/binboi.production.env.example): Go control plane production/staging env template
- [`web/.env.production.example`](./web/.env.production.example): Next.js production/staging env template
- [`docs/staging-runbook.md`](./docs/staging-runbook.md): staging bring-up and verification order
...
This is the fastest path to a working ngrok-style tunnel loop, but it should be treated as an explicit local-only mode.

1. Start the Go control plane:

```bash
export BINBOI_ALLOW_PREVIEW_MODE=true
...
Use this when you want the real product loop with accounts, access tokens, dashboard auth, and billing state. This is the recommended default for serious self-hosting.
...
- leave `BINBOI_ALLOW_PREVIEW_MODE` unset or explicitly set it to `false`

## Release checklist

- confirm `DATABASE_URL`, `AUTH_SECRET`, and `BINBOI_AUTH_DATABASE_URL`
- confirm preview mode is disabled for production-like deployments
- verify `GET /api/v1/health`, `GET /api/v1/instance`, and `GET /api/v1/tunnels`
- create one dashboard access token and validate `binboi login` plus `binboi whoami`
- expose a local app with `binboi http 3000 my-app`
- hit the public URL once and confirm the request appears in the dashboard request feed
- use [`docs/deploy-readiness.md`](./docs/deploy-readiness.md) for the full preflight and smoke sequence
...
- install channel matrix: [`docs/install-channel-matrix.md`](./docs/install-channel-matrix.md)
...
- use Homebrew or direct release binaries for end users
- use `install.sh` as a convenience wrapper over release artifacts
- use Go source builds for contributors
- do not market npm, bun, cargo, apt, or pip as supported install channels yet
