...
import { Activity, Inbox, RefreshCcw, Search } from "lucide-react";
    ...
    <div className="flex flex-col items-center gap-4 rounded-2xl border border-dashed border-white/10 px-8 py-20 text-center">
      ...
      <div>
        <p className="text-sm font-medium text-zinc-400">
          {filtered ? "No webhooks match the current filters" : "No webhook deliveries recorded yet"}
        </p>
        <p className="mt-1 text-sm text-zinc-600">
          {filtered
            ? "Clear the search or status filter to see all deliveries."
            : "Incoming webhooks (Stripe, Clerk, GitHub, etc.) will appear here every 3 seconds."}
        </p>
      </div>
    </div>
  );
...
export default function WebhooksPage() {
  // Fetch only WEBHOOK-kind records — proxied through /api/controlplane/api/v1/requests?kind=WEBHOOK
  const { requests, isLoading, isError, refresh } = useRequests({ kind: "WEBHOOK" });
  ...
  const filtered = records.filter((r) => {
    ...
    if (query.trim()) {
      ...
      const hay = [r.path, r.provider, r.event_type, r.tunnel_subdomain].join(" ").toLowerCase();
  ...
  const failed = records.filter((r) => r.status >= 400).length;
        ...
        <div className="mx-auto max-w-6xl space-y-6">
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <h1 className="mt-2 text-2xl font-semibold tracking-tight text-white">Webhooks</h1>
              <p className="mt-1.5 text-sm text-zinc-400">
                Incoming webhook deliveries routed through your tunnels from Stripe, Clerk, GitHub,
                and other providers. Refreshes every 3 seconds.
              </p>
            </div>
            <button
          ...
          {!isLoading && !isError && records.length > 0 && (
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              {[
                { label: "Total deliveries", value: records.length,             color: "text-white" },
                { label: "Avg latency",       value: `${avgMs}ms`,              color: "text-miransas-cyan" },
                { label: "Failed",            value: failed,                    color: failed > 0 ? "text-rose-400" : "text-zinc-400" },
                { label: "Providers",         value: providers.length || "—",   color: "text-[#86a9ff]" },
              ].map((s) => (
              ...
              {providers.map((p) => (
                <button
                  ...
                  className={cn(
                    ...
                    query === p
                      ? "border-[#86a9ff]/30 bg-[#86a9ff]/10 text-[#86a9ff]"
                      : "border-white/[0.07] bg-white/[0.02] text-zinc-400 hover:border-white/12 hover:text-zinc-200",
                  )}
                >
                  {p}
                </button>
          ...
          <div className="flex flex-col gap-2 sm:flex-row">
            <label className="flex flex-1 items-center gap-2 rounded-xl border border-white/[0.06] bg-[#0d0e10] px-4 py-2.5">
              ...
              <input
                ...
                placeholder="Filter by path, provider, event type, tunnel…"
          ...
          {!isLoading && !isError && records.length > 0 && (
            <p className="flex items-center gap-1.5 text-xs text-zinc-600">
              ...
              Auto-refreshing every 3 s · {records.length} delivery{records.length !== 1 ? "s" : ""} in feed
            </p>
          )}
