...
export default function TerminalLog() {
  ...
  useEffect(() => {
    ...
    socket.onopen = () => console.log("🟢 [WS_CONNECTED]: Neural Stream Active");
    
    socket.onmessage = (event) => {
      setLogs((prev) => [event.data, ...prev].slice(0, 50));
    };

    socket.onerror = (error) => console.error("🔴 [WS_ERROR]:", error);
  ...
  return (
    <div className="bg-[#080808] mt-10 border border-white/10 rounded-xl p-6 font-mono text-[10px] h-64 overflow-y-auto shadow-inner">
      ...
      <div className="space-y-2">
        ...
        {logs.length === 0 && <div className="text-gray-700 italic opacity-50">Awaiting signals from the core...</div>}
      </div>
    </div>
  );
}