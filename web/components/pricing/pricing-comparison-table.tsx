...
export function PricingComparisonTable() {
  return (
    <div className="relative overflow-hidden rounded-[2rem] border border-white/10 bg-[linear-gradient(180deg,rgba(15,21,30,0.94),rgba(7,11,17,0.98))] shadow-[inset_0_1px_0_rgba(255,255,255,0.04),0_30px_100px_rgba(0,0,0,0.28)]">
      ...
      <table className="w-full border-collapse text-left">
        <thead>
          <tr className="bg-white/[0.03] text-[10px] font-semibold uppercase tracking-[0.24em] text-zinc-500">
            <th className="sticky left-0 z-20 bg-[rgba(12,18,28,0.98)] px-5 py-4">Feature</th>
        ...
        <tbody className="divide-y divide-white/10">
          {pricingComparisonRows.map((row) => (
            <tr key={row.label} className="transition hover:bg-white/[0.02]">
              <td className="sticky left-0 z-10 bg-[rgba(10,15,23,0.98)] px-5 py-4 text-sm font-medium text-white">
                {row.label}
              </td>
              {pricingPlans.map((plan) => (
